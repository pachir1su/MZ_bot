...
@app_commands.describe(value="값(정수 또는 문자열). 승률은 % 단위, 모드명은 문자.")
@owner_only()
async def mz_config_set(interaction: discord.Interaction, field: app_commands.Choice[str], value: str):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if key == "mode_name":
            ...
            # 승률 입력이 %라면 0~100 → basis points 변환
            ...
            if key.startswith("win_"):
                ...
                num = int(v)
            if key == "win_min_bps":
                await db.execute("UPDATE guild_settings SET win_min_bps=? WHERE guild_id=?", (num, gid))
            elif key == "win_max_bps":
                await db.execute("UPDATE guild_settings SET win_max_bps=? WHERE guild_id=?", (num, gid))
            else:  # min_bet
                await db.execute("UPDATE guild_settings SET min_bet=? WHERE guild_id=?", (num, gid))
        else:
    ...
    embed.add_field(name="최소 베팅", value=f"{s[0]:,}₩")
    embed.add_field(name="승률 하한", value=f"{s[1]/100:.2f}%")
    embed.add_field(name="승률 상한", value=f"{s[2]/100:.2f}%")
    embed.add_field(name="모드명", value=s[3], inline=False)
    await interaction.response.send_message(embed=embed, ephemeral=True)
